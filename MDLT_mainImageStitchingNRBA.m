...
scale = 0.25;  % Scale of input images, 影响特征检测速度

projection = 'plane';% Possible values are: 'plane'(default), 'cylinder', or 'sphere'.
...
%------------------------------
folder_name = '/home/lynx/fuck_mount/opencv/workspace/test_result/paper/10/';
imgs_format = '*.jpg';
...
% enblend, 原算法用不了, 拿个充数
sysorder = sprintf('/home/lynx/study/stitch/enblend/build/bin/enblend --output=%s/apap.png', folder_name);

for i=1:num_imgs
    warped_img2 = imageWarping(double(ch),double(cw),double(images{i}),pairwiseH{i},double(off));
    % figure(1); imshow(warped_img2);
    % fprintf('\n%d\n', size(warped_img2));
    
    % 透明通道
    fuck_mask = reshape(double(warped_img2), size(warped_img2,1), size(warped_img2,2)/3, 3);
    fuck_mask = fuck_mask(:, :, 1);
    % figure(2); imshow(fuck_mask);
    ...
    % 保存
    fuck_path = sprintf('%s/p_%02d.png', folder_name, i);
    imwrite(warped_img2, fuck_path, 'png', 'Alpha', fuck_mask);
    % imwrite(warped_img2, fuck_path, 'png');
    sysorder = sprintf('%s %s/p_%02d.png', sysorder, folder_name, i) ;
...
system(['export LD_LIBRARY_PATH=;' sysorder], '-echo');
...
pairwiseH_fig = figure;
figure(3); imshow(canvas_img,'border','tight');% 不显示边框
drawnow;
